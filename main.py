...
class VQAModel(nn.Module):
    def __init__(self, vocab_size: int, n_answer: int, embedding_dim: int = 300, hidden_dim: int = 512):
        super().__init__()
        self.resnet = ResNet50()
        self.text_embedding = nn.Embedding(vocab_size + 1, embedding_dim)
        self.text_encoder = nn.LSTM(embedding_dim, hidden_dim, batch_first=True)

        self.fc = nn.Sequential(
            nn.Linear(512 + hidden_dim, 512),
    ...
    def forward(self, image, question):
        ...
        question_embedded = self.text_embedding(question)
        _, (question_feature, _) = self.text_encoder(question_embedded)
...
def main():
    ...
    test_dataset = VQADataset(df_path="./data/valid.json", image_dir="/valid", transform=test_transform, answer=False)
    test_dataset.update_dict(train_dataset)

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=128, shuffle=True, collate_fn=collate_fn, num_workers=os.cpu_count(), pin_memory=True)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=False, collate_fn=collate_fn, num_workers=os.cpu_count(), pin_memory=True)
    ...
    torch.save(model.state_dict(), "model_" + now.strftime('%Y%m%d_%H%M') + ".pth")
    np.save("submission_" + now.strftime('%Y%m%d_%H%M') + ".npy", submission)
